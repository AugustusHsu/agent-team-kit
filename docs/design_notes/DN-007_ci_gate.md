...
- [ ] ⭐ 出貨形式三選一（§3.1 的 A／B／C）
- [ ] ⭐ 第 1 層檢查的清單要包含哪幾項、落在哪支腳本、腳本能不能只靠標準函式庫
      （現有 `stdlib-only` job 立的那條線）
- [ ] 這些檢查要不要能在本機跑（`pre-commit` hook？還是只在 CI）
- [ ] ⭐ **「有遠端但不推送」這第三種模式**：`git_workflow.md` §8.3 只降級了「無遠端」，
      但**本 repo 就是第三種**——`origin` 存在、目前 14 顆未推送、
      而 `ci.yml` 只在 `push` 到 main 與 `pull_request` 觸發，
      所以這批 commit 的 CI **從來沒跑過**。這個中間態現在沒有任何規範接。
- [ ] CI 紅燈是否改動工單 Status（§3.3）
- [ ] kit 自己的 `ci.yml` 與出貨樣板要不要收斂成同一份
      （dogfooding 的一致性 vs 本 repo 有 kit 專屬的 `stdlib-only` 需求）

## 5. 畢業去向（畢業時填）

- 預計類型：**D（改流程／基礎建設）**——依 DN-001 §3.7，第 1 層權威在 `docs/standards/`，不需 PRD
- 預計落點：`kit/docs/standards/git_workflow.md` §8（能力對照表補「檢查內容」）、
  `kit/.github/`（視 §4 第一項裁定而定）、kit 的檢查腳本落點待定
- 預計工單：待開

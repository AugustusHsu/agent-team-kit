...
| 4.5 | ⭐ **能不能自動偵測脫節？** 例如規範章節有版本號／雜湊，skill 引用時記下，不一致就測試失敗 | 不做／章節引用檢查／覆蓋率檢查／雜湊比對 | ⭐ **能，兩條檢查都寫得出可驗收的 AC 且已用原型實跑**——見本節末的附錄。分工不同：**B 抓現況**（實跑抓出 §1.12 只有 5/13），**A 防未來**（現況 0 筆命中，防的是章節重編號後引用集體失效）。**這是本 DN 價值最高的一項**：4.1～4.4 都是一次性修補，只有這一項會持續生效。反方：兩條都只驗證「有沒有引用」，驗證不了「引用之後有沒有照做」 |
...
> ⚠️ **更正一個原本寫錯的宣稱：檢查 A 抓不到 §1.3 那一筆。**
> 本檔曾寫「A 抓得到章節引用錯置」，實跑證明是錯的——`### 1.3 Epic 關閉條件` 與
> `### 1.5 狀態更新操作方式` **兩節都真實存在**，那 4 份 skill 是「指到存在但不對的節」，
> 屬**語意錯誤**，不是懸空引用。A 只驗證「有沒有這一節」。
>
> 所以 A 的價值是**回歸防護**而非現況除錯：`team_protocol.md` 兩天內長出 §1.9～§1.12，
> 未來一旦重新編號，13 份 skill 的引用會集體失效而**沒有任何執行期錯誤**——
> A 就是為那一刻存在的。§1.3 那筆只能靠 4.2 的人工全面盤點修掉。

**A 的可選加強（A+）：引用時連章節標題一起寫。**
把慣例定成 `§1.5 狀態更新操作方式`，檢查改為「編號與標題都要對得上」。
這仍然抓不到「選錯節」（寫 `§1.3 Epic 關閉條件` 在一個講 Status 更新的段落裡，
機器看來完全一致），但它讓錯誤**對人類讀者一眼可見**，並且在章節改名時也能轉紅。
代價是 13 份 skill 的既有引用都要補標題——**併入 4.2 的盤點一起做才划算**，
單獨做不值得。是否採用併入 4.3 的撰寫標準裁定。

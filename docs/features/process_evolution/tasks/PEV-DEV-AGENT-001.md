...
- [x] **預留「並行擴充」一節**（位置在「平台適配」之前），內容目前只需
      ...
      這是為了讓日後補並行規則時**不必重排章節**，不是要現在就寫內容
...
- **✍️ User 補充回覆 (User Input)**（使用者於 2026-08-16 對話中裁定，
  ...
  **另註一（2026-08-16）**：skill 體系整體已久未更新，與現行流程的落差
  不只 git 這一項，另開 [DN-004](../../../design_notes/DN-004_skill_system_realignment.md)
  管理。本工單只處理 DN-003 裁定所涉的六份，不擴大範圍。

  **另註二（2026-08-16，並行開發的預留）**：使用者已裁定
  **「規則寫一套，內部分基本與並行擴充兩節」**——不做單人版／多人版兩份規範，
  因為多人開發包含單人開發，單線專案讀基本節就完整可用。
  並行規則本身尚在 [DN-005](../../../design_notes/DN-005_parallel_task_decomposition.md)
  與 [DN-006](../../../design_notes/DN-006_branch_topology_and_isolation.md) 探索中
  （皆為 🌱 Seed），**本工單不寫它們的內容**，只依 §3.1 最後一條預留章節位置。

  這樣切的理由是時序：本工單狀態為 `Ready`，隨時可能執行，而兩份 DN 還沒畢業。
  若不預留，DN 畢業時得回頭重排 `git_workflow.md` 的章節；若現在就寫內容，
  等於把未定案的結論送進第 1 層權威。預留一個空章節是兩者之間成本最低的選項。

  另外 DN-006 §6 已列出並行會推翻的 DN-003 裁定（squash-only、單層拓撲等）。
  **那些改動不屬於本工單**——本工單忠實落地 DN-003 的單線版本，日後由 DN-006
  的落地工單修正。

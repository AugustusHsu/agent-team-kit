...
- [ ] AC-01：`kit/CLAUDE.md` 的 Commit 範例區塊**內含 Body 條列式格式**
      （`- **標題**：說明`），不是只寫「格式見 …」。
- [ ] AC-02：範例仍維持模板自訂的長度紀律——**五行以內**（原訂三行，
      因新增格式而放寬，並在模板內寫明放寬的理由）。
- [ ] AC-03：模板要說明**取捨判準**：哪些規則必須內嵌（不寫就會做錯的：Body 格式、
      禁止 AI 署名 trailer、閘門範圍），哪些指路即可（gitmoji 對照表、範疇命名、
      互動模式操作細節）。
- [ ] AC-04：本 repo 的 `CLAUDE.md` 同步帶上 Body 格式；改動說明種子檔不受升級覆蓋。
- [ ] AC-05：新增測試，斷言 `kit/CLAUDE.md` 的 Commit 範例區塊裡出現 Body 條列式格式；
      **負向對照**：把該行從模板拿掉要能讓測試轉紅。
- [ ] AC-06：`uv run pytest` 全綠、`python3 .agent/scripts/precheck.py` 全綠。

...
- [ ] §2 裁定 5 之後有 blockquote 明說「留在 PR 內可追溯」是**前提**而非附帶效果，並指向 §6.1／§8.3
- [ ] §6.1 不再無條件 squash：拆成「有 PR／MR → squash」與「無遠端 → `--no-ff`」兩路
- [ ] §6.4 驗證方式拆兩路：squash 比對樹（維持原規則）；`--no-ff` 用 `git branch -d` 成功作為證明，且明說不必比對樹、永遠不動用 `-D`
- [ ] §8.1 能力對照表新增「合併方式（§6.1）」一列，無遠端欄為 `--no-ff`
- [ ] §8.1 的「已合併」訊號無遠端欄由「主線的樹含該分支內容」改為「`git branch -d` 成功」
- [ ] §8.1 表格說明由「這六列」改為「這七列」
- [ ] §8.3 有獨立段落說明合併方式降級的理由，並附可直接照抄的四行指令範例
- [ ] §9 檢查清單第 7 列措辭涵蓋兩種合併方式（不新增列）
- [ ] `grep -o '\[平台相關\]' kit/docs/standards/git_workflow.md | wc -l` 仍為 **9**
- [ ] `sed -n '/## 9\./,$p' kit/docs/standards/git_workflow.md | grep -c '^| .*|'` 仍為 **10**
- [ ] `grep -rn '一律 squash' kit/ | wc -l` 為 **0**（改動前為 1）
- [ ] `code-reviewer/SKILL.md` 的「不填 commit SHA」理由改為「回填當下合併還沒發生、commit 不存在」，對兩種合併方式皆成立
- [ ] `已廢除的流程規則` 新增 `一律[^\n]{0,6}squash` 一列，且**經反證**：對改動前的 `git_workflow.md` 會命中、改動後為 0
- [ ] `已廢除的流程規則` 中 `--is-ancestor` 那列的理由字串不再宣稱「§6.1 一律 squash」
- [ ] BACKLOG Icebox「worktree 重新設計」由「待開 DN」改為指向已開出的 DN-006
- [ ] `./install.sh . --upgrade` 後 `uv run pytest` 全綠，且 BACKLOG Icebox 未被 `scan_backlog.py` 清空
- [ ] 本工單以 `git merge --no-ff` 合併（即本工單新立規則的自我套用），`git branch -d` 成功

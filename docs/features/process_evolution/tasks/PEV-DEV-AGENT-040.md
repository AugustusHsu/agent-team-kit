...
- [ ] AC-01：工單模板可表達 task profile、能力 override、資料分級與 override scope。
- [ ] AC-02：舊工單無新欄位仍合法，映射結果可由 `route --explain` 顯示。
- [ ] AC-03：審查模板記錄選中 profile、候選排除理由、probe 時間與是否交接。
- [ ] AC-04：precheck 驗 shared policy schema、profile 引用與 secret-like 值未進版控。
- [ ] AC-05：precheck 不因某使用者 Claude／Codex 未登入而在 CI 轉紅。
- [ ] AC-06：負向測試涵蓋未知 profile、非法 override scope、cloud 禁止卻強制 cloud、疑似 token。
- [ ] AC-07：team_protocol 的 Role／Assignee 定義維持不變，不加入供應商角色。
- [ ] AC-08：所有安裝實例由 `./install.sh . --upgrade` 同步，待合併 0。

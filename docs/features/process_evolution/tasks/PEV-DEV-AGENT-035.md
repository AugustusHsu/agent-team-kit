...
- [ ] AC-01：共同規則只在 `AGENTS.md` 維護；`CLAUDE.md` 不複製共同段落。
- [ ] AC-02：以隔離沙盒驗證 Claude Code 能實際載入 `@AGENTS.md`，輸出留進審查紀錄。
- [ ] AC-03：五個初始 execution profiles 都有 provider、surface、capabilities、probe 與 TTL。
- [ ] AC-04：加入第三個假 provider 只需新增 manifest，核心 registry 不修改。
- [ ] AC-05：掃描 adapters 與模板，確認無 token、cookie、家目錄絕對路徑及跨供應商環境變數。
- [ ] AC-06：`.codex/` 採 allowlist 的版控邊界與 `.claude/` 原生 scope 都寫進標準。
- [ ] AC-07：必備規則集合一致性測試能抓到入口遺漏，但允許合理的供應商專屬段落。

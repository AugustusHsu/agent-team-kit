...
> **📏 篇幅例外**：本檔 199 行，超過 `docs/standards/skill_conventions.md` 第 7 節的 150 行軟上限。**理由**：Test Plan 與追溯矩陣是**可直接套用的表格樣板**，欄位定義、層級判準與範例缺一就無法照著產出。刪掉範例會把本篇從「樣板」降級成「說明」，而說明沒辦法被複製貼上。
...
- **追溯矩陣的分母只能取自第 1 層**：覆蓋率是「已覆蓋需求數 ÷ 需求總數 M」，而分母一旦取錯，算出來的百分比再漂亮也沒有意義。依 `.agent/resources/team_protocol.md` §1.11 文檔權威階序，需求母體取自 PRD／BRD 這一層；**從既有工單反推需求，會讓 M 縮成「已經有人開單做過的部分」**——沒被開單的需求連缺口都不會出現，矩陣反而顯示接近 100%，而那正是漏測最深的地方。
- **測試計畫的變更要能回答「覆蓋率為什麼變了」**：`test_plan.md` 與追溯矩陣是兩份會一起漂移的檔案，必須在同一次 commit 進版控（`.agent/resources/team_protocol.md` §1.10 Commit 閘門）。數字升降的原因（新增案例／刪除失效案例／需求本身被砍）只存在於當次的訊息裡；事後從 diff 看到的只有表格行數的增減，看不出那代表品質變好還是標準放寬。

...
> ⚠️ `git branch -d` 的「未合併」警告在 §6.1 的 squash 前提下**必然出現**——
...
| 「已合併」的訊號 | PR merged | MR merged | 主線的樹含該分支內容（§6.4） |
...
合併前手動跑完測試，`Done` 的訊號改為「主線的樹含該分支內容」（驗證方式見 §6.4），

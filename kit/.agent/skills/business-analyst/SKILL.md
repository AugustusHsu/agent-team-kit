...
- **確認你分析的是原文**：讀既有流程文件、訪談紀錄或現況資料前，先依 `.agent/resources/team_protocol.md` §1.12 取證通道保真 做開工自檢。BRD 的骨架是「ROI 數字 ＋ 痛點論證」，而這兩者的存活率天差地遠——**數字是結構化的，最容易原樣通過；論證是散文，最容易整段消失**。失真後你會留著一組漂亮的數字，卻失去了數字的來歷，而決策者要買單的正是來歷。

---

## 取材權威與產出進版控

- **BRD 的「現況」只能取自第 1 層文檔**：分析 As-Is 時最順手的材料是「上一個做完的功能是怎麼做的」，但依 `.agent/resources/team_protocol.md` §1.11 文檔權威階序，已結案工單是第 4 層，只是當時的快照。更要小心 Design Note——**DN 不在階序內**。它是探索過程的紀錄，能讀到的是「有人想過這些做法」，不是「團隊決定這麼做」。把其中一個做法寫進 BRD 的 To-Be，等於代替決策者宣布了一個從沒有人裁定過的方向，而 BRD 的整份說服力建立在「這些是既成事實」上。
- **BRD 進版控也走同一道閘門**：`brd.md` 是檔案，改它就會產生 commit，規則見 `.agent/resources/team_protocol.md` §1.10 Commit 閘門。閘門在「離開工單分支」那一刻——但 Business Analyst 經常沒有工單分支、直接在整合分支上動文件，**那種情況豁免不適用，事前同意照要**。BRD 的 commit 訊息日後會是「這個專案為什麼開始」的入口，值得寫明是哪一條商業假設變了。

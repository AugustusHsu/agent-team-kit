...
- **你讀到的輸出也可能不是原文**：工具輸出在進入你的 context 之前可能被中間層改寫，
  **表格與高度重複的結構最容易中招，表格周圍的散文可能整段消失且不留提示**。
  - **讀檔案一律用檔案讀取工具，不要用 shell 把內容印出來**（`cat`／`sed -n`）；
    非得走 shell 時，加行號前綴（例如 `cat -n`）能大幅降低被改寫的機率。
  - 出現「N 筆已壓縮」「與稍早訊息相同」這類標記，或本該是 Markdown 表格的地方
    變成 JSON 陣列——**你看到的不是原文**，換讀法重讀，不要拿它當證據。
  - 這件事關不掉，只能靠讀法規避。正版條款見 `.agent/resources/team_protocol.md` §2.3。

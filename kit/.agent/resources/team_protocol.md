...
- **審查證據來源：交付回報是線索，不是證據。**
  ...
  - **連「實際輸出」本身都要存疑**：工具輸出在進入 context 之前可能被中間層
    （壓縮 proxy、IDE 轉接、終端管線）改寫。實測過的失效形狀是**表格與高度重複的結構
    最容易被改寫，且表格周圍的散文可能整段消失而不留任何提示**。
    因此讀檔案一律用檔案讀取工具，不要用 shell 把內容印出來；
    看到內容被摘要、被去重、或本該是表格的地方變成 JSON 陣列，
    就代表**看到的不是原文**，換讀法重讀，不得拿它當審查證據。
